...
from app.api.services.dataset_service import build_merged_dataset, clean_dataset, save_processed_data
from app.core import EDA_DIR

EDA_DIR.mkdir(parents=True, exist_ok=True)


def _save_plot(fig_name: str) -> None:
    plt.tight_layout()
...
def run_eda() -> None:
    ...
    summary = {
        ...
        "top_country_by_sales": (
            df.groupby("country_region")["sales_amount"]
            .sum()
    ...
    monthly = df.set_index("order_date").resample("ME")["sales_amount"].sum().reset_index()
    plt.figure(figsize=(10, 4))
    plt.plot(monthly["order_date"], monthly["sales_amount"])
    plt.title("Monthly Sales Trend")
    plt.xlabel("Date")
    plt.ylabel("Sales Amount")
    _save_plot("sales_by_month.png")
    ...
    plt.xlabel("Region")
    plt.ylabel("Sales Amount")
    _save_plot("sales_by_region.png")
    ...
    plt.xlabel("Product")
    plt.ylabel("Sales Amount")
    _save_plot("top_products.png")

    real_customers = df[
        df["customer"].notna()
        & (df["customer"].astype(str).str.strip() != "[Not Applicable]")
    ]
    top_customers = real_customers.groupby("customer")["sales_amount"].sum().sort_values(ascending=False).head(10)
    plt.figure(figsize=(10, 4))
    top_customers.plot(kind="bar", color="#264653")
    plt.title("Top 10 Customers by Sales")
    plt.xlabel("Customer")
    plt.ylabel("Sales Amount")
    _save_plot("top_customers.png")

    with open(EDA_DIR / "summary.json", "w", encoding="utf-8") as summary_file:
